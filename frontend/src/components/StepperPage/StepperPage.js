...
const messages = defineMessages({
  ...
  prompt: {
    ...
    defaultMessage:
      'Are you sure you wanna do that? All changes will be lost if you continue!',
  },
});
...
class StepperPage extends Component {
  ...
  handleNext = () => {
    ...
    if (nextStep === STEP_SAVED) savePhoto();
    if (nextStep === STEP_FINISHED) sendData();
  };
  ...
  handleBack = () => {
    ...
    if (activeStep === STEP_FINISHED) resetForm();
    resetFinished();
  ...
  render() {
    ...
    const backButton = [STEP_USAGE].includes(activeStep) ? (
      ...
      <Button size="small" onClick={this.handleBack}>
        ...
        <span
          style={{ color: 'white', fontSize: '16px', textTransform: 'none' }}
        >
          {formatMessage(stepObj.prevBtn)}
        </span>
      </Button>
    );
    const nextButton = [STEP_CAPTURE, STEP_OVERVIEW, STEP_FINISHED].includes(
      activeStep
    ) ? (
      <div />
    ) : (
    ...
    return (
      <div className={styles.root}>
        <Prompt when={when} message={formatMessage(messages.prompt)} />
        <MainAppBar className={classes.header} />
        <div className={classes.content}>{stepObj.component}</div>
